...
function setAndPlayTrack(track) {
    ...
    playingTrack = track;
    // Cloudflare Worker streams audio directly from R2
    player.src = '/audio/' + encodeAudioPath(track);
    player.play();
    updateAllLists();
}

function encodeAudioPath(track) {
    return track.split('/').map(function (segment) {
